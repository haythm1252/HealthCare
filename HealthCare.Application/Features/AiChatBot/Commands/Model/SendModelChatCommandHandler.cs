...
public class SendModelChatCommandHandler(IUnitOfWork unitOfWork, IAiChatService aiService, ILogger<SendGeminiChatCommandHandler> logger) 
    : IRequestHandler<SendModelChatCommand, Result<AiChatResponse>>
{
    ...
    public async Task<Result<AiChatResponse>> Handle(SendModelChatCommand request, CancellationToken cancellationToken)
    {
        ...
        // model response
        var aiData = await _aiService.GetModelResponseAsync(request.Message, specialties);
        ...
        if (!string.IsNullOrWhiteSpace(aiData!.SuggestedSpecialty))
        {
            recommendedDoctors = await _unitOfWork.Doctors.AsQueryable()
                .Where(d => aiData.SuggestedSpecialty.Contains(d.Specialty.Name))
